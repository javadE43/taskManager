...
export const PatchTask = () => {
  ...
  const handleAddTask=async (e)=>{
    ...
    let res=tmdbApi.getTask(listID,task_id,config)    
    try {
      let [data]=(await res).data
      if(value.title.length>0){data.title=value.title}
      if(value.description.length>0){data.description=value.description}
      if(value.completed>0){data.completed=value.completed}
      let result=tmdbApi.patchTask(_listId,task_id,data,config)
